...
def test_upload_file():
    with open(test_file_1, 'rb') as f:
        body = f.read()
        response = client.post(api + s3_test_folder, files={"files": ('dump.sql', body, "multipart/form-data")})
        assert response.status_code == 201


def test_get_file():
    response = client.get(api + '/file' + s3_test_folder + '%2F' + test_file_name)
...
def test_update_file():
    with open(test_file_2, 'rb') as f:
        body = f.read()
        response = client.put(api + s3_test_folder + '%2F' + test_file_1,
                              files={"files": (test_file_1, body, "multipart/form-data")})
        assert response.status_code == 200
