...
@router.get("/s3_api/file/{file_path:path}",
            status_code=status.HTTP_200_OK)
async def get_file(file_path: str):
    s3 = S3Loader(file_path=file_path)
    if s3.object is not None:
        return StreamingResponse(
            s3.streamer(), media_type=s3.object['ContentType'], headers={
...
@router.get("/s3_api/directory/{dir_path:path}",
            status_code=status.HTTP_200_OK)
async def get_list_files_in_directory(dir_path: str):
    ...
    raise HTTPException(status_code=status.HTTP_204_NO_CONTENT,
                        detail="Folder isn't exist")


@router.post("/s3_api/{dir_path:path}",
             response_model=Dict,
...
async def upload_file(files: List[UploadFile],
    ...
    raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        detail="Couldn't uploaded files")


@router.put("/s3_api/{file_path:path}",
            status_code=status.HTTP_200_OK)
...
@router.delete("/s3_api/{file_path:path}",
               status_code=status.HTTP_200_OK)
